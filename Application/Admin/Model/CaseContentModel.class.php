...
use Think\Model;
class CasecontentModel extends Model 
{
	protected $insertFields = array('title','content','createtime','status','remark','pic');
	protected $updateFields = array('casecontent_id','title','content','createtime','status','remark','pic');
	...
	public function search($pageSize = 20)
	{
		...
		$data['page'] = $page->show();
		/************************************** 取数据 ******************************************/
		$data['data'] = $this->alias('a')->where($where)->group('a.casecontent_id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
	...
	{
		if(is_array($option['where']['casecontent_id']))
		{
			$this->error = '不支持批量删除';
			return FALSE;
		}
		$images = $this->field('pic,big_pic,mid_pic,sm_pic')->find($option['where']['casecontent_id']);
		deleteImage($images);
	}
	/************************************ 其他方法 ********************************************/
    public function getCasecontents($data,$page,$pageSize=10) {
    $data['status'] = array('neq',-1);
    $offset = ($page - 1) * $pageSize;
    $list = $this->_db->where($data)->order('createtime desc,casecontent_id desc')->limit($offset,$pageSize)->select();
    return $list;
    }

    public function getCasecontentsCount($data= array()) {
    $data['status'] = array('neq',-1);
    ...
    public function updateStatusById($id, $status) {
        ...
        $data['status'] = $status;
        $model=D('Casecontent');
        return $model->where('casecontent_id='.$id)->save($data);
    }
}