...
<?php endif; ?>
    public function get<?php echo $tpName; ?>s($data,$page,$pageSize=10) {
    $data['status'] = array('neq',-1);
    $offset = ($page - 1) * $pageSize;
    $list = $this->_db->where($data)->order('createtime desc,<?php echo strtolower($tpName); ?>_id desc')->limit($offset,$pageSize)->select();
    return $list;
    }

    public function get<?php echo $tpName; ?>sCount($data= array()) {
    $data['status'] = array('neq',-1);
    ...
    public function updateStatusById($id, $status) {
        ...
        return $model->where('<?php echo strtolower($tpName); ?>_id='.$id)->save($data);
    }
}