namespace <?php echo $config['moduleName']; ?>\Controller;
use \Admin\Controller\IndexController;
class <?php echo $tpName; ?>Controller extends IndexController 
{
    public function add()
    {
    	if(IS_POST)
    	{
    		$model = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
    		if($model->create(I('post.'), 1))
    		{
    			if($id = $model->add())
    			{
    				$this->success('添加成功！', U('index?p='.I('get.p')));
    				exit;
    			}
    		}
    		$this->error($model->getError());
    	}
<?php if($config['digui'] == 1): ?>
		$parentModel = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
		$parentData = $parentModel->getTree();
		$this->assign('parentData', $parentData);
<?php endif; ?>

		$this->display();
    }
    public function edit()
    {
    	$<?php echo $config['pk']; ?> = I('get.<?php echo $config['pk']; ?>');
    	if(IS_POST)
    	{
    		$model = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
    		if($model->create(I('post.'), 2))
    		{
    			if($model->save() !== FALSE)
    			{
    				$this->success('修改成功！', U('index', array('p' => I('get.p', 1))));
    				exit;
    			}
    		}
    		$this->error($model->getError());
    	}
    	$model = M('<?php echo $tpName; ?>');
    	$data = $model->find($<?php echo $config['pk']; ?>);
    	$data = $model->find($<?php echo $config['pk']; ?>);
    	$this->assign('data', $data);
<?php if($config['digui'] == 1): ?>
		$parentModel = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
		$parentData = $parentModel->getTree();
		$children = $parentModel->getChildren($<?php echo $config['pk']; ?>);
		$this->assign(array(
			'parentData' => $parentData,
			'children' => $children,
		));
<?php endif; ?>

		$this->display();
    }
    public function delete()
    {
    	$model = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
    	if($model->delete(I('get.<?php echo strtolower($tpName);?>_id', 0)) !== FALSE)
    	{
    		$this->success('删除成功！', U('index', array('p' => I('get.p', 1))));
    		exit;
    	}
    	else 
    	{
    		$this->error($model->getError());
    	}
    }
    public function index()
    {
    	$model = D('<?php echo $config['moduleName']; ?>/<?php echo $tpName; ?>');
<?php if($config['digui'] == 1): ?>
		$data = $model->getTree();
    	$this->assign(array(
    		'data' => $data,
    	));
<?php else: ?>
    $pageSize = I('get.pageSize',10,'number_int');
    	$data = $model->search($pageSize == ""?10:$pageSize);
    	$this->assign(array(
    		'data' => $data['data'],
    		'page' => $data['page'],
            'tpName' => <?php echo $tpName; ?>,
           <?php if($config['tableCnName'] == ''):?>
            'navName' => '',
            <?php else: ?>
            'navName' => <?php echo $config['tableCnName']; ?>,
            <?php endif; ?>
    	));
<?php endif; ?>
    	$this->display();
    }

    public function setStatus() {
    try {
    if ($_POST) {
    $id = $_POST['id'];
    $status = $_POST['status'];
    if (!$id) {
    return show(0, 'ID不存在');
    }
    $res = D("<?php echo $tpName; ?>")->updateStatusById($id, $status);
    if ($res) {
    return show(1, '操作成功');
    } else {
    return show(0, '操作失败');
    }
    }
    return show(0, '没有提交的内容');
    }catch(Exception $e) {
    return show(0, $e->getMessage());
    }
    }
}